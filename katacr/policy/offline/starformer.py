...
from katacr.policy.offline.resnet import ResNet, ResNetConfig
...
class StARConfig(Config):
  ...
  p_drop_resid = 0.1
  p_drop_attn = 0.1
  mode = 'star_reward_timestep'
  bar_size = BAR_SIZE
  bar_cfg = ResNetConfig(stage_sizes=[1,1,2], filters=4)
  arena_cfg = ResNetConfig(stage_size=[3,4,6], filters=16)
  n_elixir = 10
...
class StARformer(nn.Module):
  ...
  def __call__(self, s, a, r, timestep, train = True):
    ...
    bar1 = ResNet(cfg.bar_cfg)(bar1).mean(-1)[...,0,:]  # (B, N, H, W, 3)
    bar2 = ResNet(cfg.bar_cfg)(bar2).mean(-1)[...,0, :]  # (B, N, H, W, 3)
    ...
    xg = nn.Sequential([
      lambda x: ResNet(cfg.arena_cfg)(x),  # (B, N, 4, 3, 128)
