...
path_card_cls.mkdir(exist_ok=True)
GUI_MAX_HEIGHT = 1000
gui_configs = {
  'rect': {
    'info': "选框框（最多画26个）",
...
class APP:
  ...
  def _update_button(self):
    ...
    button2 = tk.Button(button_frame, text="Save Rects", font=self.TKFONT, command=self._save_rects)
    button2.pack(side="left")
  
  def _save_rects(self):
    n = len(list(path_card_cls.glob('*.jpg')))
    for i, r in enumerate(self.rectangles):
      xyxy = np.array([r.x0, r.y0, r.x1, r.y1]) / self.scale
      rect = self.img.crop(xyxy)
      path_save = path_card_cls / f"{n + i:03}.jpg"
      rect.save(path_save)
      print(f"Save {r.name} at {path_save}")
